...
Reference.prototype.fetch = function fetch() {
  this.value = (this.value && typeof this.value === 'number') ?
...
Reference.prototype.info = function info() {
  return { id: this.value.id || this.value, type: pluralize(this.relationship.type) };
};
...
One.prototype.defaultValue = function defaultValue() {
  ...
  if (val) {
    // console.log('defaultValue ' + this.parentType + '#one(' + this.type + ')', val);
  }
  return val;
};
