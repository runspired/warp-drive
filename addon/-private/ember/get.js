...
const { isPath, hasThis } = Ember.__loader.require('ember-metal/path_cache');
...
/**
 ...
 @param {String} fullPath The secret property key to retrieve
 @return {Object} the property value or `null`.
 @public
 */
export function get(obj, keyName, fullPath = keyName) {
  ...
  if (desc === undefined && isPath(keyName)) {
    return _getPath(obj, keyName, fullPath);
  }

  if (desc) {
    return desc.get(obj, keyName, fullPath);
  } else {
    ret = value;

    if (ret === undefined &&
      'object' === typeof obj && !(keyName in obj) && 'function' === typeof obj.unknownProperty) {
      return obj.unknownProperty(keyName, undefined, fullPath);
    }
...
export function _getPath(root, path, fullPath) {
  let obj = root;
